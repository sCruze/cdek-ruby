...
function loadFreshWidgetScript(url) {
    return new Promise((resolve, reject) => {
        try { delete window.CDEKWidget } catch (_) { window.CDEKWidget = undefined }
        const s = document.createElement("script")
        s.src = url
        s.async = true
        s.onload = () => resolve()
        s.onerror = () => reject(new Error("Не удалось загрузить скрипт виджета СДЭК"))
...
export default class extends Controller {
    ...
    static values = {
        ...
        fieldCoors:      { type: String, default: "order_cdek_point_coors" },
    ...
    _mountWhenReady() {
        ...
        this._mounting = true
        loadFreshWidgetScript(this.scriptUrlValue)
            .then(() => { if (this._connected) this._mountWidget() })
    ...
    _handleChoose(office) {
        ...
        const loc      = Array.isArray(office.location) ? office.location : []
        const lon      = Number(loc[0])
        const lat      = Number(loc[1])
        const coors    = (Number.isFinite(lon) && Number.isFinite(lat)) ? `${lon} ${lat}` : ""
        ...
        setFieldValue(this.fieldCoorsValue,    coors)
