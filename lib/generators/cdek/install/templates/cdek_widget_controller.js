...
//
// Про goods:
// CDEK считает тарифы по сумме габаритов и веса. Если хост-приложение
// передало JSON-массив goods через data-cdek-widget-goods-value — берём
// его, иначе используем дефолт (одна коробка 30×10×30 см / 1 кг). Дефолт
// нужен для обратной совместимости: на 0.3.5 параметра не было, и hosts
// привыкли к этим габаритам в шаблоне.

const DEFAULT_GOODS = [{ width: 30, height: 10, length: 30, weight: 1 }]
...
function parseGoods(rawJson) {
    ...
    if (trimmed === "") return DEFAULT_GOODS
    try {
