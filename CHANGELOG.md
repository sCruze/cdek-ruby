...
* Полная изоляция инстансов виджета между открытиями карты. Singleton-гард
  0.3.14 (один живой инстанс) оказался недостаточным: `@cdek-it/widget@3`
  держит ГЛОБАЛЬНЫЙ pinia-стор на уровне модуля, а `destroy()` размонтирует
  Vue-приложение, но не очищает этот стор и не снимает подписки, заведённые
  в бутстрапе виджета (вне дерева компонентов). После смены города
  (пересоздания модалки) подписки уничтоженного инстанса оставались на общем
  сторе и, реагируя на запись нового инстанса, перезапрашивали офисы и
  пере-центрировали карту на ПРЕЖНИЙ город — точки нового города уходили
  «за границы» и пропадали навсегда (в server-логах — `widget_service` по
  старому городу сразу после нового).
* Решение: шаблон `cdek_widget_controller.js` теперь перед каждым монтажом
  удаляет `window.CDEKWidget` и заново выполняет бандл, поэтому каждый виджет
  получает свежий модуль со своим стором, а подписки прежнего инстанса висят
  на осиротевшем сторе и на новый виджет не влияют. Повторное выполнение
  бандла безопасно: Yandex Maps (ymaps3) грузится идемпотентно, дублирование
  `<style>` безвредно, custom elements не регистрируются. Прежний
  `ensureWidgetScript` (один общий `_scriptPromise`) заменён на
  `loadFreshWidgetScript`.
* Шаблон генератора снова пишет координаты выбранного ПВЗ (`office.location`
  → строка `"lon lat"`) в скрытое поле `order_cdek_point_coors`. Поддержка
  `fieldCoors` была в host-копиях ≤0.3.12, но потерялась при переносе фиксов
  0.3.13/0.3.14 в шаблон — из-за этого после синхронизации контроллера
  хост-приложение переставало получать координаты пункта и не могло
  отрисовать карту заказа. Значение `fieldCoors` (по умолчанию
  `order_cdek_point_coors`) и его data-атрибут `cdek_widget_field_coors_value`
  добавлены и в шаблон, и в `cdek_widget_tag` (новый необязательный параметр
  `field_coors:`), консистентно с остальными `field_*`.
* Шаблон: после асинхронной загрузки скрипта виджета монтирование
  выполняется только если контроллер ещё подключён
  (`if (this._connected) this._mountWidget()`). Раньше промис загрузки мог
  доехать уже после `disconnect()` (быстрое закрытие/переоткрытие модалки),
  и виджет монтировался в уже отсоединённый узел.
* Совокупно с 0.3.13 (центрирование карты по координатам вместо асинхронного
  геокодинга названия) и 0.3.14 (ровно один живой инстанс) это полностью
  убирает «мелькание» и пропадание точек ПВЗ при открытии карты после смены
  города. Хост-приложениям, сгенерировавшим контроллер на ≤0.3.15, нужно
  перегенерировать его (`bin/rails generate cdek:install`) или внести правку
  вручную из шаблона.
